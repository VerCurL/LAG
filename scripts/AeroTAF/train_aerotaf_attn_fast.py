...
from algorithms.utils.AeroTAF_ATNN_Fast import AeroTAFATTNFastBase
...
class PPOAeroTAFATTNFast(nn.Module):
...
def forward_losses(model, batch, device, args):
    ...
    for seq_len, samples in grouped.items():
        ...
        threat_preds.append(threat_output)
        attack_preds.append(attack_output)
...
def get_parser():
    parser = argparse.ArgumentParser(description="Train AeroTAF_ATNN_Fast on detailed point-index datasets.")
...
def main(args):
    ...
    model = PPOAeroTAFATTNFast(AeroTAFATTNFastArgs(all_args), obs_space, act_space, device=device)
    ...
    best_ckpt_path = run_dir / "AeroTAF_ATNN_Fast_best.pt"
    latest_ckpt_path = run_dir / "AeroTAF_ATNN_Fast_latest.pt"
    ...
    for epoch in range(1, all_args.epochs + 1):
        ...
        logging.info(
            ...
            f"train_active={len(datasets['train'])} factual-only | "
