...
3. 从完整对局中最多选择 `n_rollout_threads` 局；通常对应每个并行环境各取一局。
4. 使用 `FieldCalculator` 计算每个时间点的 K 步威胁场和进攻场标签。
5. 在这 32 局上拟合 detail 阈值，并按以下优先级标注时间点：
...
8. 对 rollout 中每个时间点推理事实场值；逐架飞机替换当前动作，再推理反事实场值。
...
- `--n-rollout-threads 32`：并行环境数量，同时决定每轮最多用于阈值拟合和 AeroTAF 训练的完整对局数。
