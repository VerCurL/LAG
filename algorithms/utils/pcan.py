...
class PCANLayer(nn.Module):
    ...
    def __init__(self, agent_num: int, head_num: int, KQ_input_dim: int, V_input_dim: int, activation_id,
        ...
        self._output_hidden_size = [self._V_hidden_size[-1]] + list(map(int, output_hidden_size.split(' '))) + [V_input_dim]
        ...
        self.output_module = MLPLayer(self._V_hidden_size[-1], output_hidden_size + " " + str(V_input_dim), activation_id)
