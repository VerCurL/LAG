...
class PPOTrainer():
    ...
    def ppo_update(self, policy: PPOPolicy, sample):
        ...
        if active_masks_batch.sum().item() < 1:
            return None
        ...
        active_count = active_masks_batch.sum().clamp(min=1e-6)
        policy_loss = -(policy_action_loss * active_masks_batch).sum() / active_count
        ...
        policy_entropy_loss = -(dist_entropy * active_masks_batch).sum() / active_count
    ...
    def train(self, policy: PPOPolicy, buffer: SharedReplayBuffer):
        ...
        for _ in range(self.ppo_epoch):
            if self.use_recurrent_policy:
                data_generator = buffer.recurrent_generator(buffer.advantages_mask, self.num_mini_batch, self.data_chunk_length)
            else:
                raise NotImplementedError

            for sample in data_generator:

                out = self.ppo_update(policy, sample)
                if out is None:
                    continue

                policy_loss, value_loss, policy_entropy_loss, ratio, actor_grad_norm, critic_grad_norm = out
